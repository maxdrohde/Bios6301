...
```
The gender coefficent seems to have the highest effect on `los` because the value is largest.
```
...
```
```
The estimate for the intercept is 3.5
The estimate for gender is 4.3
```
...
summary(mod) %>% vcov %>% diag %>% sqrt

# We see that the standard error computed with this method
# agrees with those given by the `coef()` function
```
...
```{r}
# Store the t-statistics for gender
t_stat_gender <- mod.c[,3][2]
pt(q=t_stat_gender, df=7, lower.tail = FALSE)*2

# The calculated p-value of 0.02243214 agrees with the value in the table
```
...
```{r}
(deviance(mod)/df.residual(mod)) %>% sqrt()

summary(mod)

# The Residual standard error is 2.197 using both methods
```
...
```{r}
dplyr::filter(x, gender=="M") -> men
dplyr::filter(x, gender=="F") -> women
```
...
# The t.test with var.equal=TRUE produces output that matches the p-value
# from the models summary
```
